...
def bfs(grid, start, target, wall):
    ...
    while queue:
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] not in wall and (nr, nc) not in visited:  # ! = wall 에서 not in wall 로 변경
                visited.add((nr, nc))
...
START_SYMBOL = 'M'
TARGET_SYMBOL = 'X'
WALL_SYMBOL = {'R','W'}  # 물 지형 추가, 이동 가능한 지형만 최단 경로로 이동해야 하므로 'W' 를 WALL-SYMBOL 에 추가
